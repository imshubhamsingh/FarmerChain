...
class Login extends Component {
		...
		if (this.state.confirmPassword === this.state.signUpPassword) {
			...
			axios
				...
				.then(function(response) {
					...
					// console.log(data)
					that.props
						.createAccount(signUpEmail, signUpPassword, signUpDisplayName, data)
						.then(() => {
	...
	renderLogin = () => {
		return (
			<div
				className={'form ' + (this.state.login ? 'remove-form' : '') + (this.state.error ? 'error' : '')}
				style={{ display: this.props.user.loading ? 'block' : 'none' }}
			>
				...
				<div className={'form-panel two ' + (this.state.signUp ? 'show' : '')} onClick={this.openSignUp}>
					<div className="form-toggle" onClick={this.closeSignUp} />
					<div className="form-header">
					...
					<div className="form-content">
						<form onSubmit={this.signUp}>
							<div className="form-group">
								...
								<input
									type="text"
									id="userSignUpDisplayName"
									name="username"
									required="required"
									value={this.state.signUpDisplayName}
									onChange={(event) => this.setState({ signUpDisplayName: event.target.value })}
								/>
							</div>
							<div className="form-group">
